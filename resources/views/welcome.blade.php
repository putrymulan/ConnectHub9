...
<body>
                    ...
                    @foreach ($categories as $data)
                        <div class="card">
                            <div class="card-image">
                                <img src="{{('./img_kategori/' . $data->gambar) }}" alt="tidak ada gambar" />
                            </div>
                            <div class="card-content">
                                <h5>{{ $data->nama }}</h5>
                                <p class="description">{{ $data->deskripsi }}</p>
                                <p class="price">{{ $data->harga }}</p>
                                <button class="btn_belanja" type="button" onclick='bukaModal({{ $data->id_categories }})'>Beli</button>
                            </div>
                        </div>
                        <!-- Modal 1 -->
                        <div id="myModal{{ $data->id_categories }}" class="modal-container" onclick="tutupModal({{ $data->id_categories }})">
                            <div class="modal-dialog" onclick="event.stopPropagation()">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title" style="color: #ffb72b;">Form Transaksi</h1>
                                        <button type="button" class="btn-close" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form>
                                            <input type="hidden" name="category_id" id="category_id" value="{{ $data->id_categories }}">
                                            ...
                                            <div class="form-group">
                                                <label class="labelmodal" for="seat-text" class="col-form-label">Seat:</label>
                                                <textarea class="inputseat" class="form-control" id="seat-text"></textarea>
                                            </div>
                                        </form>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" onclick="tutupModal({{ $data->id_categories }})">Keluar</button>
                                        <button type="button" class="btn btn-yellow" onclick="bukaModal2({{ $data->id_categories }})">Lanjutkan</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Modal 2 -->
                        <div id="myModal2{{ $data->id_categories }}" class="modal-container" onclick="tutupModal2({{ $data->id_categories }})">
                            <div class="modal-dialog" onclick="event.stopPropagation()">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title" style="color: #ffb72b;">Data Transaksi</h1>
                                        <button type="button" class="btn-close" aria-label="Close" onclick="tutupModal2({{ $data->id_categories }})"></button>
                                    </div>
                                    <form action="{{ route('information') }}" method="post">
                                        ...
                                        <div class="modal-body">
                                            <h4>Kategori</h4>
                                            <div class="form-group">
                                                <input type="hidden" name="id_categories" value="{{ $data->id_categories }}">
                                                <label class="labelmodal" for="detail-kategori">Kategori :</label>
                                                <input class="inputdata" type="text" name="detail-kategori" class="form-control" id="detail-kategori" value="{{ $data->nama }}" readonly>
                                            </div>
                                            <div class="form-group">
                                                <label class="labelmodal" for="detail-harga">Harga :</label>
                                                <input class="inputdata" type="text" name="detail-harga" class="form-control" id="detail-harga" value="{{ $data->harga }}" readonly>
                                            </div>
                                            <h4>Pembeli</h4>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                <label class="labelmodal" for="detail-seat" class="col-form-label">Seat:</label>
                                                <input class="inputdata" name="detail-seat" class="form-control" id="detail-seat" readonly>
                                                @error('detail-seat')
                                                    <p style="font-size: 10px; color: red">{{ $message }}</p>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" onclick="kembaliKeModalPertama({{ $data->id_categories }})">Kembali</button>
    ...
    <script>
        ...
        function lakukanPembayaran() {
            ...
            document.getElementById("seat-text").value = "";
        }
    </script>
</body>
